...
#-----------------------------------------------------
#--------- Code from Henry Segerman ------------------
#-----------------------------------------------------
# Sends Pixels to CP1 and back. Code available on:
...
# Functions generating the np.matrixes
...
def rotate_around_axis_sphere_points_p_q(p,q,theta):
    ...
    assert np.dot(p,q) < 0.9999, "axis points should not be in the same place!"
    CP1p, CP1q = CP1_from_sphere(p), CP1_from_sphere(q)
    r = get_vector_perp_to_p_and_q(p,q)
...
def give_marix_from_numbers(a,b,c,d):
    return np.matrix([[a,b],[c,d]])

def give_marix_from_string(a,b,c,d):
    
    a = parse_to_complex(a)
    b = parse_to_complex(b)
    c = parse_to_complex(c)
    d = parse_to_complex(d)
    
    return np.matrix([[a,b],[c,d]])

def parse_to_complex(a):
    a = a.replace(' i','1i')
    a = a.replace('+i','+1i')
    a = a.replace('-i','-1i')
    a = a.replace('i','j')
...
def apply_matrix_to_image(M_SL2C, in_loc, out_loc, out_size=None):
    
    src_image = np.array(Image.open(in_loc),dtype=np.float32)
    ...
    np.clip(o_im,0,255,o_im)
    Image.fromarray(o_im.astype(np.uint8)).save(out_loc)